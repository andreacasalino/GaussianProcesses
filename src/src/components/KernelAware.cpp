...
namespace gauss::gp {
    ...
    namespace {
        Eigen::MatrixXd compute_kernel_portion(const std::vector<Eigen::VectorXd>& samples, const KernelFunction& function,
    ...
    void KernelAware::updateKernel() {
        ...
        if (nullptr == kernel) {
            kernel = std::make_unique<Eigen::MatrixXd>(compute_kernel_portion(input_samples, *kernelFunction,
                MatrixIndices{0, input_samples.size()}, MatrixIndices{ 0, input_samples.size() }));
        }
        else {
            MatrixIndices old_indices = MatrixIndices{0, kernel->rows()};
            MatrixIndices new_indices = MatrixIndices{kernel->rows(), input_samples.size()};
            Eigen::MatrixXd K_old_new = compute_kernel_portion(input_samples, *kernelFunction, old_indices, new_indices);
            Eigen::MatrixXd K_new_new = compute_kernel_portion(input_samples, *kernelFunction, new_indices, new_indices);
            ...
            set_matrix_portion(*new_kernel, K_old_new.transpose(), new_indices, old_indices);
