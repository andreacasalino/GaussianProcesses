...
namespace gauss::gp {
    ...
    protected:
        ...
        GaussianProcessBase(KernelFunctionPtr kernel, const std::size_t input_space_size,
            const std::size_t output_space_size);

        // row wise
        Eigen::VectorXd getKx(const Eigen::VectorXd& point) const;
        void predict(const Eigen::VectorXd& point, Eigen::VectorXd& mean, double& covariance) const;
